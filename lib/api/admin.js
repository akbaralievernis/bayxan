...
const STUB_STAFF = [
  { id: "s-1", name: "Байхан Асанов",   role: "admin",   title: "Владелец",                department: "admin",   phone: "+996 553 30-04-01", pin: "0000", is_admin: true,  active: true,  hired_at: "2017-04-12" },
  { id: "s-2", name: "Эльдар Бекбаев",  role: "manager", title: "Старший администратор",   department: "admin",   phone: "+996 700 111 222",  pin: "9012", is_admin: true,  active: true,  hired_at: "2019-09-01" },
  { id: "s-3", name: "Айгуль Сатарова", role: "cook",    title: "Су-шеф",                  department: "kitchen", phone: "+996 770 555 333",  pin: "5678", is_admin: false, active: true,  hired_at: "2020-03-15" },
  { id: "s-4", name: "Нурлан Каримов",  role: "cook",    title: "Повар",                   department: "kitchen", phone: "+996 555 222 111",  pin: "1234", is_admin: false, active: true,  hired_at: "2022-06-20" },
  { id: "s-5", name: "Алия Жунушева",   role: "waiter",  title: "Старший официант",        department: "hall",    phone: "+996 700 333 444",  pin: "2222", is_admin: false, active: true,  hired_at: "2021-02-10" },
  { id: "s-6", name: "Тимур Абдылдаев", role: "waiter",  title: "Официант",                department: "hall",    phone: "+996 770 444 555",  pin: "3333", is_admin: false, active: true,  hired_at: "2023-01-05" },
  { id: "s-7", name: "Назгуль Орозова", role: "cashier", title: "Кассир",                  department: "hall",    phone: "+996 555 666 777",  pin: "4444", is_admin: false, active: true,  hired_at: "2022-11-15" },
];
...
export async function createStaff(input) {
  ...
  if (!supabase) {
    if (STUB_STAFF.some((s) => s.pin === payload.pin)) {
      return { ok: false, error: "PIN уже занят другим сотрудником" };
    }
...
export async function updateStaff(id, input) {
  ...
  if (!supabase) {
    ...
    if (idx === -1) return { ok: false, error: "not found" };
    // Disallow duplicate PIN
    const conflict = STUB_STAFF.find((s) => s.pin === payload.pin && s.id !== id);
    if (conflict) return { ok: false, error: "PIN уже занят другим сотрудником" };
...
function sanitizeStaffPayload(input) {
  return {
    ...
    pin:        String(input.pin || "").trim().replace(/\D/g, "").slice(0, 4),
