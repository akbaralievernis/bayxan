...
export default function DishCard({ dish }) {
  ...
  const inCart = useCartStore((s) =>
    s.items.find((i) => i.id === dish.id)?.quantity ?? 0
  );
  ...
  const seed = dish.id.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-4">
        ...
        <p className="text-[13px] leading-relaxed text-stone-500 line-clamp-2 min-h-[2.6em]">
          {dish.description}
        </p>

        <div className="mt-2 flex items-center justify-between gap-3">
          <div className="font-display font-bold text-xl text-amber-700 tabular-nums">
